...
NCountry = {
	...
	SPECIAL_FORCES_CAP_BASE = 0.0,					-- Max ammount of special forces battalions is total number of non-special forces battalions multiplied by this and modified by a country modifier
	SPECIAL_FORCES_CAP_MIN = 150,					-- You can have a minimum of this many special forces battalions, regardless of the number of non-special forces battalions you have, this can also be modified by a country modifier
...
NProduction = {
	...
	BASE_FACTORY_MAX_EFFICIENCY_FACTOR = 25,	-- Base max efficiency for factories expressed in %.
	...
	EQUIPMENT_MODULE_ADD_XP_COST = 1.0,					-- XP cost for adding a new equipment module in an empty slot when creating an equipment variant.
	EQUIPMENT_MODULE_REPLACE_XP_COST = 1.0,				-- XP cost for replacing one equipment module with an unrelated module when creating an equipment variant.
...
NMilitary = {
	...
	CORPS_COMMANDER_DIVISIONS_CAP = 35,			-- how many divisions a corps commander is limited to. 0 = inf, < 0 = blocked
	...
	FIELD_MARSHAL_DIVISIONS_CAP = 35,			-- how many divisions a field marshall is limited to. 0 = inf, < 0 = blocked
	...
	RELIABILITY_ORG_REGAIN = -0.45,                 -- how much reliability affects org regain
	RELIABILITY_ORG_MOVING = -1.5,                 -- how much reliability affects org loss on moving
	...
	BASE_LEADER_TRAIT_GAIN_XP = 0.4,			   -- Base xp gain for traits per hour for armies
...
NAir = {
	...
	AIR_WING_ATTACK_LOGISTICS_TRUCK_DAMAGE_FACTOR = 0.2,
	...
	AIR_WING_ATTACK_LOGISTICS_TRAIN_DAMAGE_FACTOR = 0.03,
	...
	AIR_WING_ATTACK_LOGISTICS_TRUCK_MAX_FACTOR = 0.01, -- max trucks we can destroy in one instance of a logistics strike
...
NNavy = {
	...
	XP_GAIN_FACTOR = 0.7,	   			   							-- xp gain factor for navy
	...
	NAVAL_MINES_IN_REGION_MAX = 750.0,								-- Max number of mines that can be layed by the ships. The value should be hidden from the user, as we present % so it's an abstract value that should be used for balancing.
	...
	NAVAL_ACCIDENT_CHANCE_REDUCTION_ON_POTF = 0.005,					-- Scale of the current chance for an accident to happen, applied for the pride of the fleet.
	...
	SUB_DETECTION_CHANCE_BASE = 6,									-- to start spotting a submarine, a dice is rolled and checked if it succeeds this percentage. if not, that enemy sub force won't be spotted on this tick
	...
	HIGHER_SHIP_RATIO_POSITIONING_PENALTY_FACTOR					= 0.3, -- if one side has more ships than the other, that side will get this penalty for each +100% ship ratio it has
	MAX_POSITIONING_PENALTY_FROM_HIGHER_SHIP_RATIO					= 0.6,  -- maximum penalty to get from larger fleets
	...
	MAX_POSITIONING_PENALTY_FOR_NEWLY_JOINED_SHIPS 					= 0.8,  -- the accumulated penalty from new ships will be clamped to this value
	POSITIONING_PENALTY_HOURLY_DECAY_FOR_NEWLY_JOINED_SHIPS			= 0.2,-- the accumulated penalty from new ships will decay hourly by this value
	...
	CAPITAL_RATIO_FOR_FULL_SCREENING_FOR_CARRIERS 					= 2.0,  -- this capital ratio to num carriers is needed for full screening beyond screen line
	...
	LIGHT_GUN_ATTACK_TO_SHORE_BOMBARDMENT							= 0.02, -- light gun attack value is divided by this value * 100 and added to shore bombardment modifier
	...
	CONVOY_DEFENSE_MAX_CONVOY_TO_SHIP_RATIO							= 24.0,		-- each ship in convoy defense mission can at most cover this many convoys without losing efficiency
	...
	NAVAL_COMBAT_AIR_SUB_DETECTION_MAX = 8.0,
	NAVAL_COMBAT_AIR_SUB_DETECTION_SLOPE = 8.0,						-- lower means sharper curve (ramps up very fast, then flatten out very fast). Must be >0
...
NRailwayGun = {
	...
	ATTACK_TO_FORTS_MODIFIER_FACTOR = 1.3,		-- Forts modifier is calculated by multiplying railway gun attack value with this and dividing by 100
	ATTACK_TO_ENTRENCHMENT_MODIFIER_FACTOR = 0.9,		-- Entrenchment modifier is calculated by multiplying railway gun attack value with this and dividing by 100
...
NIndustrialOrganisation = {
	...
	DESIGN_TEAM_CHANGE_XP_COST = 2,								-- Flat cost added to the XP cost of a new equipment design. If 0, cost is entirely disabled.
